...
function setData(date) {
    if (typeof date === 'string') {
        ...
        let month = dateShow.innerHTML = `${now.getDate()}.${now.getMonth() === 0 ? now.getMonth() + 1 : now.getMonth()}.${now.getFullYear()}`;
...
function getWeather(day) {
    ...
    fetch(weatherUrl + `${chouseCity.latitude}, ${chouseCity.longitude}&data=${day}`).then(res => res.json()).then(weatherResult);
}
...
function searchLocation() {
    fetch(searchUrl + inputSearch.value).then(blonb => blonb.json()).then(data => {
        if (data.data) {
